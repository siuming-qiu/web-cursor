...
| DB 驱动 | `@neondatabase/serverless` + drizzle `neon-http` | **Vercel 多实例**：HTTP 无长连接，不会打爆 Postgres 连接数（普通 `pg` 连接池在 serverless 下会因实例数膨胀而耗尽连接） |
...
// 不再读 MAX、不再用事务（单条 INSERT，neon-http 也能直接跑）。
...
export async function POST(req, { params }) {
  ...
  const msg = await appendMessage(params.id, body)   // 复用 ① 的事务版 seq 计算
  return ok(msg, 201)
}
```
...
| **连接数被打爆**：每实例开 TCP 连接，Postgres `max_connections` 撑不住 | 用 **Neon serverless driver（HTTP，无长连接）**，不是 `pg` 连接池（见技术栈表）。 |

```ts
// lib/db/index.ts —— 多实例友好的 DB 客户端
import { neon } from '@neondatabase/serverless'
import { drizzle } from 'drizzle-orm/neon-http'
import * as schema from './schema'

const sql = neon(process.env.DATABASE_URL)   // 用池化连接串；HTTP 调用，不持有连接
...
> neon-http 驱动**不支持多语句事务**。本方案所有 DB 操作都是单条语句（seq 改 identity 后 `appendMessage` 不再需要事务），正好契合。将来万一需要真事务，把那一条路径单独换成 Neon 的 WebSocket `Pool`。
