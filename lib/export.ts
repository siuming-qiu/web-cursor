...
/** 源码 → 自包含可运行 HTML。代码用 data URL 引入，彻底避开 </script> 转义问题。 */
export async function buildExportHtml(source: string, title: string): Promise<string> {
  const js = await transpile(source);
  const dataUrl = `data:text/javascript;base64,${toBase64(js)}`;
  return `<!doctype html>
<html lang="zh-CN">
<head>
<meta charset="utf-8">
